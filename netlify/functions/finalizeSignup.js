...
exports.handler = async (event) => {
    ...
    switch (stripeEvent.type) {
        case 'customer.subscription.created': { // FIRES FIRST
            const subscription = stripeEvent.data.object;
            const { placeId, email } = subscription.metadata;
            const userId = subscription.customer;

            console.log('Subscription created, creating customer record...');
            try {
                // This block creates the user record immediately so the dashboard can load.
                const customerDoc = await db.collection('customers').doc(userId).get();
                if (customerDoc.exists) break;

                const signupsRef = db.collection('signups');
                const snapshot = await signupsRef.where('googlePlaceId', '==', placeId).orderBy('timestamp', 'desc').limit(1).get();
                if (snapshot.empty) throw new Error(`No matching signup doc for placeId: ${placeId}`);
                ...
                const customerData = {
                    ...
                    reviewInvitesSent: 0,
                    signupDate: new Date(),
                    subscriptionStatus: 'active', // Assumes it will become active
                };
        ...
        case 'invoice.paid': { // FIRES AFTER SUCCESSFUL PAYMENT
            const invoice = stripeEvent.data.object;
            
            // Only send email for the VERY FIRST invoice of a new subscription
            if (invoice.billing_reason === 'subscription_create') {
                ...
                try {
                    const customerDoc = await db.collection('customers').doc(userId).get();
                    if (!customerDoc.exists) throw new Error('Customer record not found for sending email.');
